...
def b1248_lm_scheme(args):
    ...
    args.sum2sum_self = getattr(args, "sum2sum_self", "default")
    base_lm_scheme(args)
...
def base_lm_architecture(args):
    ...
    args.normalize_before = getattr(args, "normalize_before", True)  # Model training is not stable without this
...
def museformer_lm_v2s1(args):
    ...
    args.use_token_in_chunk_abs_pos = getattr(args, 'use_token_in_chunk_abs_pos', True)
    args.learned_token_in_chunk_abs_pos = getattr(args, 'learned_token_in_chunk_abs_pos', False)

    args.use_token_rel_pos = getattr(args, 'use_token_rel_pos', True)
    args.learned_token_rel_pos = getattr(args, 'learned_token_rel_pos', True)
    args.no_token_rel_pos_for_prefix = getattr(args, 'no_token_rel_pos_for_prefix', True)
    args.no_token_rel_pos_proj = getattr(args, 'no_token_rel_pos_proj', True)

    args.use_bar_rel_pos = getattr(args, 'use_bar_rel_pos', True)
    args.learned_bar_rel_pos = getattr(args, 'learned_bar_rel_pos', True)
    args.no_bar_rel_pos_proj = getattr(args, 'no_bar_rel_pos_proj', True)
    args.valid_parts_for_bar_rel_pos = getattr(args, 'valid_parts_for_bar_rel_pos', ('rs', 'rr'))
...
@register_model_architecture("museformer_lm", "museformer_lm_v3")
def museformer_lm_v3(args):
    args.attention_mode = getattr(args, 'attention_mode', 'sum_then_reg_2')
    ...
    args.attn_query_proj_bias = getattr(args, 'attn_query_proj_bias', False)
    args.attn_key_proj_bias = getattr(args, 'attn_key_proj_bias', False)
    args.attn_value_proj_bias = getattr(args, 'attn_value_proj_bias', False)
    args.attn_out_proj_bias = getattr(args, 'attn_out_proj_bias', True)

    base_lm_architecture(args)


@register_model_architecture("museformer_lm", "museformer_lm_v5")
def museformer_lm_v5(args):
    args.attention_mode = getattr(args, 'attention_mode', 'sum_then_reg_3')
    args.share_reg_kv_proj = getattr(args, 'share_reg_kv_proj', True)
    museformer_lm_v3(args)
