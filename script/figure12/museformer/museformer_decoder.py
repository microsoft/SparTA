...
class MuseformerDecoder(FairseqDecoder):
    ...
    def add_args(cls, parser):
        # === Implementation ===
        parser.add_argument('--attention-impl', choices=('mask', 'blocksparse', 'sparta', "triton", "deepspeed"))
        ...
        parser.add_argument('--valid-dict-len', type=int)
        ...
        # relative token position embedding
        parser.add_argument('--use-token-rel-pos', type=arg_tools.str_bool_with_default_error)
        parser.add_argument('--learned-token-rel-pos', type=arg_tools.str_bool_with_default_error)
        parser.add_argument('--max-token-rel-pos', type=int)
        parser.add_argument('--token-rel-pos-embed-dim', type=int)
        parser.add_argument('--no-token-rel-pos-for-prefix', type=arg_tools.str_bool_with_default_error)
        parser.add_argument('--no-token-rel-pos-proj', type=arg_tools.str_bool_with_default_error)
        ...
        # relative bar position embedding
        parser.add_argument('--use-bar-rel-pos', type=arg_tools.str_bool_with_default_error)
        parser.add_argument('--learned-bar-rel-pos', type=arg_tools.str_bool_with_default_error)
        parser.add_argument('--max-bar-rel-pos', type=int)
        parser.add_argument('--bar-rel-pos-embed-dim', type=int)
        parser.add_argument('--no-bar-rel-pos-proj', type=arg_tools.str_bool_with_default_error)
        parser.add_argument('--valid-parts-for-bar-rel-pos', type=arg_tools.comma_split_tuple_of_specific_type(str))

        # relative position embeddings
        parser.add_argument('--rel-proj-bias', type=arg_tools.str_bool_with_default_error)
    ...
    def __init__(self, args, dictionary):
        ...
        if self.attention_impl in ['blocksparse', 'deepspeed']:
        ...
        if self.attention_mode == 'simu':
            self.attn_mask_combination = 'all'
            raise NotImplementedError
        elif self.attention_mode == 'sum_then_reg':
            self.attn_mask_combination = {'sx': ('ss', 'sr'), 'rx': ('rs', 'rr')}
            raise NotImplementedError
        elif self.attention_mode == 'v2s1':
            self.attn_mask_combination = {'sx': ('ss', 'sr'), 'rx': ('rs', 'rr')}
        elif self.attention_mode == 'sum_then_reg_2':
            self.attn_mask_combination = {'rx': ('rs', 'rr')}
            self.attn_mask_gen_parts = ('sr', 'rs', 'rr')
        elif self.attention_mode == 'sum_then_reg_3':
            self.attn_mask_combination = None
            self.attn_mask_gen_parts = ('sr', 'rs', 'rr')
        else:
            raise NotImplementedError
        self.need_embedding_summary_tokens = True
        if self.attention_mode in ('sum_then_reg_2', 'sum_then_reg_3'):
            self.need_embedding_summary_tokens = False
        ...
        # if self.attention_impl == 'sparta':
        #     assert not self.args.fp16, "sparta cannot be applied when using fp16"
    ...
    def build_decoder_layer(
        ...
        attention_dropout,
        token_rel_pos_embeddings=None,
        bar_rel_pos_embeddings=None,
        **kwargs
    ):
        return MuseformerDecoderLayer(
            ...
            layer_attention_scheme,
            attention_dropout,
            token_rel_pos_embeddings=token_rel_pos_embeddings,
            bar_rel_pos_embeddings=bar_rel_pos_embeddings,
            **kwargs
        )
    ...
    def extract_features(
        ...
        **kwargs
        ...
        # print(self.pocket_constant)
        # print(self.pocket_instant)
        # print('------')
        # print()
        # input()
        ...
        # if self.attention_impl == 'sparta':
        #     assert not self.training, "sparta cannot be applied in training mode, yet"

        # ===== Instant Info Construction ===
        # Construct the missing input by the default scheme. Only for inference.
        # Todo: This part is only for temporarily use, and not for general situations.
        ...
        # with open('reg_chunk_ranges.bin', 'wb') as f:
        #     torch.save(reg_chunk_ranges, f)
        #     print('saved reg_chunk_ranges')

        # ===== Summary Sequence and Embeddings =====
        max_comp_chunk = int(num_complete_chunks.max())
        sum_len = max_comp_chunk * self.num_summary
        # sum status:
        # When self.num_summary == 0: don't use sum
        # When self.num_summary > 0:
        #   When self.need_embedding_summary_tokens:
        #     Embed sum now
        #   When not self.need_embedding_summary_tokens:
        #     Don't embed sum here, but it exists.
        ...
        if self.use_token_abs_pos or self.use_token_rel_pos:
        ...
        if self.use_token_rel_pos:
            self.pocket_instant['reg_token_pos_ids'] = token_abs_pos_ids
        del token_abs_pos_ids

        # --- Absolute Bar Position Embedding ---
        sum_bar_pos_ids = None
        reg_bar_pos_ids = None
        if self.use_bar_abs_pos or self.use_bar_rel_pos:
        ...
        if self.use_bar_rel_pos:
            self.pocket_instant['sum_bar_pos_ids'] = sum_bar_pos_ids
            self.pocket_instant['reg_bar_pos_ids'] = reg_bar_pos_ids
