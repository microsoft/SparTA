...
# from .datasets.chunk_sequence_dataset import ChunkSequenceDataset
from .datasets.chunk_sequence_dataset_2 import ChunkSequenceDataset as ChunkSequenceDataset2
# from .datasets.prefix_dataset import PrefixDataset
from .datasets.prefix_tag_dataset import PrefixTagDataset
# from .datasets.music_monolingual_dataset import MusicMonolingualDataset
...
# from .datasets.truncate_music_dataset import TruncateMusicDataset
...
class MuseformerLanguageModelingTask(LanguageModelingTask):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        if getattr(self.args, 'eoc_token', None) is not None:
            assert self.args.eoc_token in self.dictionary, '%s not in dictionary' % self.args.eoc_token
            eoc_index = self.dictionary.index(self.args.eoc_token)
        else:
            eoc_index = None
