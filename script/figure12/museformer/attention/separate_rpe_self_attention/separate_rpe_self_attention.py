...
class SeparateRpeSelfAttention(nn.Module):
    def __init__(
        self,
        embed_dim,
        num_heads,
    ...
    def forward(
        self,
        x: torch.Tensor,  # (all_seq_len, bsz, embed_dim)  all_seq_len == temp_sum_len + reg_len
        sum_len,
        reg_len,
        rel_indices,  # relation list of (bsz, all_seq_len, all_seq_len) or dict of parts (bsz, len, len)
        key_padding_mask=None,  # (bsz, all_seq_len)
        attn_mask=None,  # (bsz * num_heads, all_seq_len, all_seq_len) or (all_seq_len, all_seq_len)
        ...
        *args, **kwargs,
        ...
        for rel_index in rel_indices:
            assert isinstance(rel_index, dict) or rel_index.shape == (bsz, all_seq_len, all_seq_len)
        ...
        attn_weights = self.do_qk_scores(
            base_q, base_k, rel_qs, r_list, rel_indices,
            bsz, sum_len, reg_len, all_seq_len, attn_mask=attn_mask
        )
        ...
        attn_weights = self.do_masking(
            attn_weights, attn_mask,
            bsz, all_seq_len
        )

        attn_weights_float = self.do_attn_softmax_float(attn_weights, attn_mask=attn_mask)
        attn_weights = attn_weights_float.type_as(attn_weights)
        attn_probs = self.dropout_module(attn_weights)  # (bsz, num_heads, all_seq_len, all_seq_len)
        attn_weights = None
        ...
        # with open('attn_mask_at_layer_%d.pt' % self.layer_idx, 'wb') as f:
        #     if isinstance(attn_mask, (tuple, list)):
        #         first, second = attn_mask
        #         first = first.cpu()
        #         second = second.cpu()
        #         torch.save((first, second), f)
        #         del first, second
        #     else:
        #         torch.save(attn_mask.cpu(), f)

        del attn_mask
        del attn_probs
        attn = self.do_out_proj(attn, sum_len, reg_len)
