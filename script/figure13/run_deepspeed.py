...
from transformers.models.opt.modeling_opt_deepspeed_training import OPTDecoderLayer, OPTForCausalLM
import deepspeed
...
def main():
    ...
    def tokenize_function(examples):
        # torch.save(examples, "debug.pt")
        # assert False
        with CaptureLogger(tok_logger) as cl:
            # [f"{a} {b} {c}" for a, b, c in zip(examples["instruction"], examples["input"], examples["output"])]
            output = tokenizer([f"{a} {b} {c}" for a, b, c in zip(examples["instruction"], examples["input"], examples["output"])], padding="max_length", max_length=1024, truncation=True)
    ...
    def module_inject(layer_obj, model, config, micro_batch_size, max_seq_length, seed, preln, fp16=False):
        ...
        return model
    model = module_inject(OPTDecoderLayer, model, config, training_args.per_device_train_batch_size, 128, training_args.seed, config.do_layer_norm_before, fp16=False)
    ...
    if training_args.do_train:
        ...
        with open("results.txt", "a") as f:
            f.write(f"{model_args.model_name_or_path},DeepSpeed,{end - st},{memory}\n")
