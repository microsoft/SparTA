...
extern "C" __global__ void nmsparse_vw4_gemv_simt_fp32_fp32_fp32_32x128x128_8x4(float *g_vec, float *g_mat_data, int *g_mat_index, float *g_data, const int M, const int N, const int K, const float sparsity)
{
...
namespace nmsparse {
	...
	cudaError_t nmsparseSpMMVW4(nmsparseContext_t ctx, int m, int k, int n, dtype *mat_a_dense, int *mat_b_sparse_idx, dtype *mat_b_sparse_val, dtype *output, cudaStream_t stream)
	{
		...
		nmsparse_vw4_gemv_simt_fp32_fp32_fp32_32x128x128_8x4<<<dimGrid, dimBlock, shared_mem_size>>>(mat_a_dense, mat_b_sparse_val, mat_b_sparse_idx, output, M, K, N, sparsity);
