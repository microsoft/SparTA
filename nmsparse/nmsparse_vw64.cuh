#ifndef _NMSPARSE_KERNEL_VECTORWISE64_H_
#define _NMSPARSE_KERNEL_VECTORWISE64_H_
...
namespace nmsparse
{
	__global__ void nmsparse_vw64_gemm_tensorop_fp32_fp32_fp32_32x32x32_4x4(float *g_vec, float *g_mat_data, int *g_mat_index, float *g_data, const int M, const int N, const int K, const float sparsity)
	{
...
#pragma unroll
	...
	__global__ void nmsparse_vw64_gemm_tensorop_fp32_fp32_fp32_32x32x64_4x4(float *g_vec, float *g_mat_data, int *g_mat_index, float *g_data, const int M, const int N, const int K, const float sparsity)
	{
		const int BLOCK_SIZE_M = 32;
		const int BLOCK_SIZE_N = 32;
...
#pragma unroll
	...
	cudaError_t nmsparseSpMMVW64(nmsparseContext_t ctx, int m, int k, int n, dtype *mat_a_dense, int *mat_b_sparse_idx, dtype *mat_b_sparse_val, dtype *output, cudaStream_t stream = 0);

	template <typename dtype>
	cudaError_t nmsparseSpMMVW64(nmsparseContext_t ctx, int m, int k, int n, dtype *mat_a_dense, int *mat_b_sparse_idx, dtype *mat_b_sparse_val, dtype *output, cudaStream_t stream)
	{
		assert(is_divisible(m, 32) && is_divisible(n, 32) && is_divisible(k, 32));
		...
		if (sparsity == 0.5f){
			...
			nmsparse_vw64_gemm_tensorop_fp32_fp32_fp32_32x32x32_4x4<<<dimGrid, dimBlock, shared_mem_size>>>(mat_a_dense, mat_b_sparse_val, mat_b_sparse_idx, output, M, N, K, sparsity);
		}else{
			const int BLOCK_SIZE_M = 32;
			const int BLOCK_SIZE_N = 32;
			const int BLOCK_SIZE_K = 64;
			...
			dim3 dimGrid(M / BLOCK_SIZE_M, N / BLOCK_SIZE_N);
			int shared_mem_size = sizeof(float) * (BLOCK_SIZE_M * BLOCK_SIZE_K_SPARSE + BLOCK_SIZE_N * BLOCK_SIZE_K_SPARSE);
			nmsparse_vw64_gemm_tensorop_fp32_fp32_fp32_32x32x64_4x4<<<dimGrid, dimBlock, shared_mem_size>>>(mat_a_dense, mat_b_sparse_val, mat_b_sparse_idx, output, M, N, K, sparsity);
		}
