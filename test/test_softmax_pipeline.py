...
factory = specializer.get_factory('sparse_softmax')
...
def prepare_data():
    ...
    C_exp = np.exp(C) * C_mask_tiled
    C_exp_sum = C_exp.sum(axis=-1).reshape((-1, 1)) + 1e-10
    C_tgt = C_exp / C_exp_sum
    C_tgt = C_tgt.astype(np.float32)
    return C, C_val, C_mask, C_tgt

C, C_val, C_mask, C_tgt = prepare_data()

# Test Function
test_func = factory.get_test_interface(cfg, mask={'C_in': C_mask})
print(f'NVCC Latency: {test_func(inputs={"C_in": C, "C_out": C_tgt}, num_iters=1000)} ms')

# PyTorch Module
module_interface = factory.get_module_interface(cfg, mask={'C_in': C_mask})
