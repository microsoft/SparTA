...
kernel = kernels.OpenAITemplateSparseMatMulKernel('sdd', biased=False, transpose=False)
...
def prepare_data():
    ...
    A *= A_mask_tiled
    C_tgt = A @ B
    return A, A_val, A_mask_tiled, B, C_tgt

A, A_val, A_mask, B, C_tgt = prepare_data()

# Test Function
test_latency = kernel.test(cfg, mask={'A': A_mask}, inputs={'A': A, 'B': B}, num_iters=1000)
